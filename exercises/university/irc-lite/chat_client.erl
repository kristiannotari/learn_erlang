-module(chat_client).
-export([connect/5, start/1]).

start(Nick) ->
    connect("localhost", 2223, "AsDT67aQ", "general", Nick).
...
disconnected(Group, Nick) ->
    receive
      ...
      {status, S} ->
	  io:format("~p~n", [S]), disconnected(Group, Nick);
      Other ->
	  io:format("chat_client disconnected unexpected:~p~n",
		    [Other]),
	  disconnected(Group, Nick)
    end.
...
start_connector(Host, Port, Pwd) ->
    S = self(),
    spawn_link(fun () -> try_to_connect(S, Host, Port, Pwd)
	       end).
...
active(MM) ->
    receive
      {msg, Nick, Str} ->
	  lib_chan_mm:send(MM, {relay, Nick, Str}), active(MM);
      {chan, MM, {msg, From, Pid, Str}} ->
	  io:format("~p@~p: ~p~n", [From, Pid, Str]), active(MM);
      {close, MM} -> exit(serverDied);
      Other ->
	  io:format("chat_client active unexpected:~p~n",
		    [Other]),
	  active(MM)
    end.
